...
The package command creates `artifacts/live-webview-1.0.0.vsix` and `artifacts/webview-dev-helper-1.0.0.tgz`. The VSIX has no runtime package dependencies. Sample JSON files join fixture-ready acknowledgements with the companion's receipt logs. They report build time separately and do not enforce timing thresholds in CI.

Linux CI is checked in. Local validation results and any unverified criteria are recorded in [validation.md](validation.md).

Live Webview supports local desktop VS Code with a Node extension host. It does not support remote hosts, browser VS Code, arbitrary attachment, production-target overrides, or host hot swapping. Message inspection, sample scenarios, and Vite HMR remain later stages.
